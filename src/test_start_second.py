...
select_item = browser.find_element_by_css_selector(
    "tr[data-index='2'] td[class='bs-checkbox'] label[class='checkbox-wrap']")
select_item.click()
time.sleep(1)
stopbutton = browser.find_element_by_xpath("//button[@action='stop']")
startbutton = browser.find_element_by_xpath("//button[@action='start']")
situation = browser.find_element_by_css_selector("tr[data-index='2'] span")

if situation.get_attribute(
        "class") == "host-list-icon iconfont icon-state-ctrlplay":
    stopbutton.click()
    time.sleep(1)
...
if select_items[4].get_attribute("checked") is None:
    ...
    delete_button.click()
    browser.find_element_by_css_selector(".btn.btn-submit.btn-primary")

#edit cdn operation
if select_items[5].get_attribute("checked") is None:
    select_items[5].click()
    time.sleep(1)
    more_button=browser.find_element_by_css_selector(".btn.btn-small.dropdown-toggle")
    delete_button=browser.find_element_by_css_selector(".btn.dropdown-item.btn-host-force-shut-down")
    edit_button=browser.find_element_by_css_selector(".btn.dropdown-item.btn-host-reset")
...
def origin_doamin():
    ...
    domainname_value = "www." + ''.join(random.sample('abcdefghijklmnopqrstuvwxyz', length)) + ".com"
