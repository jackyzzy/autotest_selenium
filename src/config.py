...
ComputeGroup = [
    ...
    cfg.StrOpt('ssh_user',
               default='root',
               help="User name used to authenticate to an instance."),
    # NOTE(fty): add ssh validation and cirros password to authenticate
    cfg.StrOpt('ssh_cirros_password',
               default='cubswin:)',
               help="cirros_password used to authenticate to an instance."),
    cfg.BoolOpt('ssh_run',
                default=False,
...
NetworkGroup = [
    ...
    cfg.StrOpt('public_network_id',
               ...
               help="Id of the public network that provides external "
                    "connectivity"),
   cfg.StrOpt('public_service_network_id',
               default="",
               help="Id of the public service  network that provides public service network  "
                    "connectivity"),
...
VolumeGroup = [
    ...
    cfg.IntOpt('build_timeout',
               ...
               help='Timeout in seconds to wait for a volume to become '
                    'available.'),
    cfg.IntOpt('list_iscsi_timeout',
               default=30,
               help='Timeout in seconds to wait for a iscsi target to become '
                    'available.'),
    cfg.StrOpt('catalog_type',
               default='volume',
...
ServiceAvailableGroup = [
    ...
    # NOTE FTY: add billcenter
    cfg.BoolOpt('billcenter',
                default=True,
                help="Whether or not billcenter is expected to be available"),

    cfg.BoolOpt('nodemgr',
                default=True,
                help="Whether or not nodemgr is expected to be available"),

    cfg.BoolOpt('sf_cdn',
                default=True,
                help="Whether or not sf_cdn is expected to be available"),

    cfg.BoolOpt('sfdev',
                default=True,
                help="Whether or not sfdev is expected to be available")
]
...
sf_notify_group = cfg.OptGroup(name='sf_notify',
                               title='Sf_notify Services Options')

SfNotifyGroup = [
    cfg.StrOpt('catalog_type',
               default='sf_notify',
               help="Catalog type of the Sf_notify service."),
    cfg.StrOpt('region',
               default='RegionOne',
               help="The sf_notify region name to use. Also used as the other "
                    "services' region name unless they are set explicitly. "
                    "If no such region is found in the service catalog, the "
                    "first found one is used."),
    cfg.StrOpt('endpoint_type',
               ...
               choices=['public', 'admin', 'internal',
                        'publicURL', 'adminURL', 'internalURL'],
               help="The endpoint type to use for the sf_notify service.")
]

sf_common_info_group = cfg.OptGroup(name='sf_common_info',
                                    title='Sf_notify Services Options')

SfCommonInfoGroup = [
    cfg.StrOpt('catalog_type',
               default='sf_common_info',
               help="Catalog type of the sf_common_info service."),
    cfg.StrOpt('ctl_host',
               default=None,
               help='the ip of the control node'),
    cfg.StrOpt('billcenter_host',
               default=None,
               help='the ip of the control node'),
    cfg.StrOpt('ctl_ssh_username',
               default='root',
               help='the ssh username of control node'),
    cfg.StrOpt('ctl_ssh_password',
               default='1',
               help='the ssh password of control node'),
    cfg.IntOpt('ctl_ssh_port',
               default=22,
               help='the ssh username of control node'),
    cfg.StrOpt('horizon_username',
               default='admin',
               help='the horizon username of openstack'),
    cfg.StrOpt('horizon_password',
               default='1',
               help='the horizon password of openstack')
]

# old value sf_vsan_info
sf_vsan_info_group = cfg.OptGroup(name='vs',
                                  title='VS Services Options')
SfVsanInfoGroup = [
    cfg.IntOpt('build_interval',
               default=10,
               help='Time in seconds between volume availability checks.'),
    ...
    cfg.StrOpt('catalog_type',
               default='cluster',
               help="Catalog type of the vs service."),
    cfg.StrOpt('region',
               default='',
               help="The vs region name to use. Also used as the other "
                    "services' region name unless they are set explicitly. "
                    "If no such region is found in the service catalog, the "
                    "first found one is used."),
    cfg.StrOpt('endpoint_type',
               ...
               choices=['public', 'admin', 'internal',
                        'publicURL', 'adminURL', 'internalURL'],
               help="The endpoint type to use for the vs service."),
    cfg.ListOpt('cluster_hosts',
                default=["node-450.acloud.vt", "node-451.acloud.vt"],
                help="List of hosts for vsan cluster"
                ),
    cfg.ListOpt('expand_hosts',
                default=["node-449.acloud.vt"],
                help="List of hosts for vsan cluster expand"
                ),
    cfg.IntOpt('replica_num',
               default=2,
               help="the number of replica in vsan cluster"
               )
]

sfprocess_group = cfg.OptGroup(name='sfprocess',
                               title='Sfprocess Services Options')

SfprocessGroup = [
    cfg.StrOpt('catalog_type',
               default='sfprocess',
               help="Catalog type of the Sfprocess service."),
    cfg.StrOpt('region',
               default='RegionOne',
               help="The sfprocess region name to use. Also used as the other "
                    "services' region name unless they are set explicitly. "
                    "If no such region is found in the service catalog, the "
                    "first found one is used."),
    cfg.StrOpt('endpoint_type',
...
sfpool_group = cfg.OptGroup(name='sfpool',
                            title='Sfpool Services Options')

SfpoolGroup = [
    cfg.StrOpt('catalog_type',
               default='volume',
               help="Catalog type of the Sfpool service."),
    cfg.StrOpt('region',
               default='RegionOne',
               help="The sfpool region name to use. Also used as the other "
                    "services' region name unless they are set explicitly. "
                    "If no such region is found in the service catalog, the "
                    "first found one is used."),
    cfg.StrOpt('endpoint_type',
               ...
               choices=['public', 'admin', 'internal',
                        'publicURL', 'adminURL', 'internalURL'],
               help="The endpoint type to use for the sfpool service.")
]

billcenter_group = cfg.OptGroup(name='billcenter',
                                title='billcenter Service Options')

BillcenterGroup = [
    cfg.StrOpt('catalog_type',
               default='billcenter',
               help="Catalog type of the Billcenter service."),
    cfg.StrOpt('region',
               default='RegionOne',
               help="The Billcente region name to use. Also used as the other "
                    "services' region name unless they are set explicitly. "
                    "If no such region is found in the service catalog, the "
                    "first found one is used."),
    cfg.StrOpt('endpoint_type',
               ...
               choices=['public', 'admin', 'internal',
                        'publicURL', 'adminURL', 'internalURL'],
               help="The endpoint type to use for the Billcenter service."),
    cfg.BoolOpt('too_slow_to_test',
                default=True,
                help="This variable is used as flag to enable "
                     "notification tests")
...
nodemgr_group = cfg.OptGroup(name='nodemgr',
                             title='nodemgr Service Options')

NodemgrGroup = [
    cfg.StrOpt('catalog_type',
               default='nodemgr',
               help="Catalog type of the nodemgr service."),

    cfg.StrOpt('region',
               default='RegionOne',
               help="The nodemgr region name to use. Also used as the other "
                    "services' region name unless they are set explicitly. "
                    "If no such region is found in the service catalog, the "
                    "first found one is used."),
    cfg.StrOpt('endpoint_type',
               ...
               choices=['public', 'admin', 'internal',
                        'publicURL', 'adminURL', 'internalURL'],
               help="The endpoint type to use for the Nodemgr service."),
    cfg.BoolOpt('too_slow_to_test',
                default=True,
                help="This variable is used as flag to enable "
                     "notification tests")
...
cdn_group = cfg.OptGroup(name='cdn',
                         title='cdn Service Options')

CdnGroup = [
    cfg.StrOpt('catalog_type',
               default='sf_cdn',
               help="Catalog type of the cdn  service."),

    cfg.StrOpt('region',
               default='RegionOne',
               help="The sf_cdn region name to use. Also used as the other "
                    "services' region name unless they are set explicitly. "
                    "If no such region is found in the service catalog, the "
                    "first found one is used."),
    cfg.StrOpt('endpoint_type',
...
sfdev_group = cfg.OptGroup(name='sfdev',
                             title='Sfdev Service Options')

SfdevGroup = [
    cfg.StrOpt('catalog_type',
               default='sfdev',
               help="Catalog type of the sfdev service."),

    cfg.StrOpt('region',
               default='RegionOne',
               help="The sfdev region name to use. Also used as the other "
                    "services' region name unless they are set explicitly. "
                    "If no such region is found in the service catalog, the "
                    "first found one is used."),
    cfg.StrOpt('endpoint_type',
               ...
               choices=['public', 'admin', 'internal',
                        'publicURL', 'adminURL', 'internalURL'],
               help="The endpoint type to use for the Sfdev service."),
    cfg.BoolOpt('too_slow_to_test',
                default=True,
                help="This variable is used as flag to enable "
                     "notification tests")
...
qos_group = cfg.OptGroup(name='sf_qos',
                         title='qos Service Options')

QosGroup = [
    cfg.StrOpt('image_name',
               default='centos7_std_perfkit_big',
               help="Create image that contain qos tool."),
    cfg.IntOpt('qos_test_time',
               default=15,
               help="Time to wait for detect the qos."),
    cfg.StrOpt('download_server',
               default='http://200.200.115.123/other/CentOS-7-x86_64-GenericCloud.qcow2',
               help="Download server."),
    cfg.FloatOpt('qos_precision',
               default=0.05,
               help="default to 5%"),
    cfg.StrOpt('dest_server',
               default='200.200.112.123',
               help="Dest server."),
    cfg.StrOpt('dest_server_2',
               default='200.200.112.124',
               help="Dest server."),
    cfg.StrOpt('dest_server_password',
               default='Linux',
               help="Password of dest server."),
    cfg.StrOpt('dest_http_path',
               default='/usr/local/nginx/html/',
               help="Path of dest http server."),
    cfg.StrOpt('dest_big_file',
               default='CP.m2t',
               help="Path of dest http server."),
    cfg.StrOpt('curl_loader_config',
               default='/home/perfkit/curl_loader_conf/10K.conf',
               help="default of curl-loader config."),
]


dnat_group = cfg.OptGroup(name='sf_dnat',
                         title='dnat Service Options')

DnatGroup = [
    cfg.StrOpt('mount_pass',
               default='jiayou88',
               help="password to mount share file."),
]
...
_opts = [
    ...
    (sf_notify_group, SfNotifyGroup),
    (sf_common_info_group, SfCommonInfoGroup),
    (sf_vsan_info_group, SfVsanInfoGroup),
    (sfprocess_group, SfprocessGroup),
    (sfpool_group, SfpoolGroup),
    (billcenter_group, BillcenterGroup),
    (nodemgr_group, NodemgrGroup),
    (cdn_group, CdnGroup),
	(sfdev_group, SfdevGroup),
    (qos_group, QosGroup),
    (dnat_group, DnatGroup),
    (None, DefaultGroup)
]
