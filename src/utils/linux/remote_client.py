...
class RemoteClient(object):
    ...
    def sf_get_ram_size_in_mb(self):
        output = self.exec_command('cat /proc/meminfo | grep MemTotal')
        if output:
            get_mem = output.split()[1]
            if get_mem:
                LOG.debug("+++sf, get_mem = %s", get_mem)
                return ((int(get_mem) + (1<<20)) >> 20) << 10
    ...
    def ping_host(self, host, count=CONF.compute.ping_count,
        ...
        cmd += ' -c{0} -w{1} -s{2} {3}'.format(count, timeout, size, host)
        return self.exec_command(cmd)
